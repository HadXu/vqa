...
def question2seq(texts_list,wordvec,seq_len):
    ...
    tokenizer = Tokenizer(num_words=1e8)
...
def get_result(y_pred, output,multi_mode=True,is_tr=False):
...
class DataLoader:
    ...
    def __init__(self,training,data,y,cfg):
        ...
        with open(data_path + 'q2a.json', 'r') as f:
            self.q2a = json.loads(f.read())
        with open('./info/te_distr.json', 'r') as f:
            self.te_distr = json.loads(f.read())
    ...
    def get_data_tr_(self,split=1,offset=0,training=None,idx=None):
        ...
        if training:
            ...
            while self.curr_index < self.num_samples:
                ...
                batch_idx = range(curr_i,min(next_i,self.num_samples))
                y, data = self.sampling_question(batch_idx)
                data.update({
